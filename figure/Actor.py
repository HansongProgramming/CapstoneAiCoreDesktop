...
# Link the brush collection to the scene collection for visibility in the Outliner
    ...
    bpy.ops.transform.resize(value=(5.08686, 5.08686, 5.08686), orient_type='GLOBAL', orient_matrix=((1, 0, 0), (0, 1, 0), (0, 0, 1)), orient_matrix_type='GLOBAL', mirror=False, use_proportional_edit=False, proportional_edit_falloff='SMOOTH', proportional_size=1, use_proportional_connected=False, use_proportional_projected=False, snap=False, snap_elements={'INCREMENT'}, use_snap_project=False, snap_target='CLOSEST', use_snap_self=True, use_snap_edit=True, use_snap_nonedit=True, use_snap_selectable=False)
...
def register():
    ...
    bpy.app.handlers.load_post.append(setup_context)
...
def unregister():
    ...
    bpy.app.handlers.load_post.remove(setup_context)

def setup_context(scene):
    # Ensure that the canvas object exists before proceeding
    canvas = bpy.data.objects.get("Plane")  # Make sure the name matches
    ...
    if canvas:
        for area in bpy.context.screen.areas:
            if area.type == 'PROPERTIES':
                ...
                # Check if Dynamic Paint modifier exists on the canvas object
                if "Dynamic Paint" in canvas.modifiers:
                    bpy.ops.dpaint.output_toggle(override, output='A')
                    bpy.ops.dpaint.output_toggle(override, output='B')
                    print("✔ Successfully toggled Output A & B!")
                else:
                    print("⚠ Dynamic Paint modifier not found on the canvas object!")

                break
        else:
            print("⚠ Could not find the PROPERTIES area!")
    else:
        print("⚠ Canvas object not found!")
